...
class Order:
    ...
    def to_entity(cls, doc: Dict) -> "Order":
        # support your existing DB key totalPrice if it exists
    ...
    def from_entity(self) -> Dict:
        # NOTE: repository converts supplier_id/business_id/item_id to ObjectId where appropriate
        doc = {
            "supplier_id": self.supplier_id,
            "business_id": self.business_id,
            "estimated_eta": self.estimated_eta,
            "ordered_items": [x.from_entity() for x in self.ordered_items],
            "totalPrice": self.total_price,           # keep DB schema stable
