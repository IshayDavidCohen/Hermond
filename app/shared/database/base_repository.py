...
class BaseRepository:
    ...
    def _get_document(
        self, document_id: ObjectId = None, query: Dict = None
    ) -> Optional[Dict]:
        if document_id:
    ...
    def _get_documents_by(
        self, query: Dict, additional_query: Optional[Dict] = None
    ) -> Cursor:
        return self._db.find_all(
            collection=self._collection, query=query, subfield_query=additional_query
        )
    ...
    def _update_document(self, document_id: ObjectId, update_data: Dict) -> bool:
        ...
        result = self._db.update_one(
            self._collection, {"_id": document_id}, update_data
        )
    ...
    def _upsert_subfield(self, document_id: ObjectId, field_query: Dict) -> bool:
        result = self._db.update_one(
            self._collection, {"_id": document_id}, field_query, operation="$set"
        )
        return result.modified_count > 0
    ...
    def _remove_subfield(self, document_id: ObjectId, field_query: str) -> bool:
        ...
        result = self._db.update_one(
            self._collection, query, update, operation="$unset"
        )
        return result.modified_count > 0
