...
from app.domain.entities.Order import Order, OrderedItem, OrderStatus
from app.utilities.funcs import to_oid

class OrderRepository(BaseDAO):
    ...
    def _convert_order_doc_ids_to_mongo(doc: Dict) -> Dict:
        """Convert supplier_id/business_id and ordered_items[].item_id to ObjectId for Mongo storage."""
        doc = doc.copy()
    ...
    def update_active_order_fields(self, order_id: str, update_data: Dict) -> bool:
        """Patch fields without overwriting the full document."""
        oid = to_oid(order_id)
        if not oid:
            return False
    ...
    def archive_active_order(self, order_id: str) -> bool:
        """Move doc from active_orders to order_history. (Non-transactional unless you add sessions.)"""
        oid = to_oid(order_id)
        if not oid:
            return False
