...
@router.post(
    "/create",
    response_model=CreateHandshakeResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_handshake(
    body: CreateHandshakeRequest,
...
@router.get(
    "/get/user_handshakes/{user_type}/{user_id}",
    response_model=List[HandshakeResponse],
)
async def get_user_handshakes(
    user_type: str,
    user_id: str,
    service: HandshakeService = Depends(get_handshake_service),
):
...
async def respond_handshake(
    ...
    ok = service.respond_to_handshake(
