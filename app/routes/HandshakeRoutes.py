...
def create_handshake_route():
    handshake_agent = current_app.config['handshake_agent']
    ...
    if check_dict_validity(validity_map=HANDSHAKE, data=creation_data):
        ...
        hid, created = handshake_agent.new_handshake(sender_id=creation_data['sender_id'],
...
def get_handshake_route(handshake_id):
    handshake_agent = current_app.config['handshake_agent']
    return jsonify(handshake_agent.handshake_module.get_handshake(handshake_id)), 201


@handshake_bp.route('/get/user_handshakes/<user_type>/<user_id>', methods=['GET'])
def get_user_handshakes_route(user_type, user_id):
    handshake_agent = current_app.config['handshake_agent']
    handshakes = handshake_agent.get_user_handshakes(user_id, user_type)
    if handshakes:
...
def ack_handshake_routes(user_id, handshake_id, response):
    ...
    if response in STATUS_OPTIONS:
        return jsonify({'success': handshake_agent.process_handshake(user_id, handshake_id, response)}), 201
    return 'Invalid response', 400

# @supplier_bp.route('/get/<supplier_id>', methods=['GET'])
# def get_supplier_route(supplier_id):
#     supplier_agent = current_app.config['supplier_agent']
#     return jsonify(supplier_agent.supplier_module.get_supplier(supplier_id)), 201
#
#
# @supplier_bp.route('/get/category_carousel/<category_id>', methods=['GET'])
# def get_category_carousel(category_id):
#     supplier_agent = current_app.config['supplier_agent']
#     return jsonify(supplier_agent.get_supplier_carousel(category_id=category_id)), 201
#
#
...
#         returned_id = supplier_agent.create_item(creation_data)
...
#     supplier_agent = current_app.config['supplier_agent']
#     return jsonify(supplier_agent.delete_item(item_id)), 201
