...
from app.ordering.domain.entities.order import Order, OrderedItem, OrderStatus
...
class OrderRepository(BaseRepository, IOrderRepository):
    ...
    def _convert_order_doc_ids_to_mongo(doc: Dict) -> Dict:
        """Convert supplier_id / business_id and ordered_items[].item_id to ObjectId."""
        doc = doc.copy()
    ...
    def archive_active_order(self, order_id: str) -> bool:
        """Move doc from active_orders to order_history."""
        oid = to_oid(order_id)
        if not oid:
            return False
